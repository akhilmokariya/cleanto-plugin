                ...
                if(mysqli_num_rows($addons_data) > 0){
                    while($adonsdata =mysqli_fetch_array($addons_data)){
                        ...
                        if($adonsdata['multipleqty'] == 'N'){
                            ...
                            <li class="ct-sm-6 ct-md-4 ct-lg-3 ct-xs-12 mb-15">
                                <?php
                                if(isset($addonrates_data['rules']) && $addonrates_data['rules']=='G')
								{
									$strate=$addonrates_data['rates'];
									$fg = 1;
									$fe=0;
								}
								if($fg == 1)
								{
									if(isset($addonrates_data['rules']) && $addonrates_data['rules'] == 'E')
									{
										?>
										<span class="duration-box">
											<input type="checkbox" name="addon-checkbox" class="addon-checkbox add_addon_in_cart_for_multipleqty" data-status="1" data-duration_value="-1" data-id="<?php echo $adonsdata['id']; ?>" id="ct-addon-<?php echo $adonsdata['id']; ?>" data-rate="<?php echo ($calculation_policy == "M") ? $adonsdata['base_price'] : $adonsdata['base_price']*$i; ?>" data-service_id="<?php echo $adonsdata['service_id']; ?>" data-method_id="0" data-method_name="<?php echo $adonsdata['addon_service_name']; ?>" data-units_id="<?php echo $adonsdata['id']; ?>" data-type="<?php echo "addon"; ?>" data-mnamee="<?php echo $mmnameee; ?>" checked />
										</span>
										<?php 
								...
								}else if(sizeof($_SESSION['ct_cart']['method']) > 0) {
                                    foreach ($_SESSION['ct_cart']['method'] as $addon) {
                                    $flag = 0;
